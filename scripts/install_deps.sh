...
uname -v >/dev/null 2>&1 || {
    echo >&2 "ERROR - solidity requires 'uname' to identify the platform."
    exit 1
}
...
Darwin)
    ...
    *)
    ...
    brew -v >/dev/null 2>&1 || {
        echo >&2 "ERROR - solidity requires a Homebrew install.  See https://brew.sh."
        exit 1
    }
    brew update
    brew install boost
    ...
    Ubuntu | LinuxMint | Pop)
        ...
        *)
        ...
        if [ "$CI" = true ]; then
            ...
            if ! dpkg -l libz3-dev >/dev/null 2>&1; then
